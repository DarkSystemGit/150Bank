...
async function main(ws, data) {
   ...
   if (data.type === `signUp`) {
      ...
      ws.send(JSON.stringify({
         res: fuzzySearch(compileProductData(), data.name, 0.3)
      }))
      ...
      try {
         ...
         if (data.hasOwnProperty('name')) {
            database.Companies[data.orgName].name = data.name
         }
         if (data.hasOwnProperty('description')) {
            database.Companies[data.orgName].description = data.description
         }
